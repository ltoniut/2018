...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            randomizeName() {
                const engName = axios.get('api/donjon.bin.sh/fantasy/random/rpc.cgi', { params: { type: 'Kingdom Name', n:"1" } })[0];
            },
            recommendCityNames() {
                this.recommendedCities = axios.get('api/donjon.bin.sh/fantasy/random/rpc.cgi', { params: { type: 'Common Town Name', n:"1" } });
            },
            recommendFauna() {
                const animalArray = [];
                let randomNmb = 0;

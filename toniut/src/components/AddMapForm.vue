...
<script>
    ...
    methods: {
        ...
        getManyNames() {
            this.exampleNames = axios.get('api/donjon.bin.sh/name/rpc.cgi', { params: { type: 'Draconic Male', n:"10" } });
        },
        randomizeName() {
            this.name = axios.get('api/donjon.bin.sh/name/rpc.cgi', { params: { type: 'Draconic Male', n:"1" } })[0];
        },
        submitForm() {
        if (
            this.name !== "" &&
